...
void draw()
{
  ...
  if (siRectPlayClick( rectPlayPosCouin, rectPlayTailX, rectPlayTailY) == true)
  {
    background(0, 0, 100);    
  }
...
boolean siRectPlayClick(int l_rectPlayPosCouin, int l_rectPlayX, int l_rectPlayY)
{
  boolean rcPlay = false;
  
    if ((mousePressed &&
       (mouseX > l_rectPlayPosCouin)) &&
       (mouseX < (l_rectPlayPosCouin + l_rectPlayX)) && 
       (mouseY >= height/17) && 
       (mouseY <= ((height/17)+l_rectPlayY)))
  {
    rcPlay = true;
  }
  return rcPlay;
